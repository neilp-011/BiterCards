exports.handler = async function () {
  ...
  try {
    const res = await fetch('https://reseller.fazercards.com/api/v2/giftcards', {
    ...
    if (!res.ok) {
      return { statusCode: res.status, body: JSON.stringify({ error: data.message || 'Error al leer el catálogo' }) };
    }
